...
以下是两张软件演示的动图，第一次加载可能需要点时间（如果动图无法加载，可以直接去当前仓库目录下：images->1.gif以及2.gif ）。
...
* 以ASC方式发送数据到MCU。以这种方式方式数据时要注意，当在发送数据框中输入键盘的换行符时，换行符会一并发送到MCU。

* 以HEX方式发送数据到MCU。每个十六进制值之间以**空格**隔开，不足两位的十六进制**不需要补0**，十六进制也**不区分大小写**。
    > 01 02 03 04 ad
    或者
    > 1 2 3 4 AD
