...
#### <保存全部数据>功能该怎么用？
点击**保存全部数据**按钮完成了两件事。一个是将RTT_T2的数据区内的log全部保存到txt文本，另一个是打开这个txt文本。所以如果你将**txt文本**的**默认打开程序**（通过windows系统）设置为**notepad**或者**其他你喜欢的编辑器**，你可以强大的编辑器中搜索、编辑log，这比rtt_t2自带的文本搜索强大的多。
